...
export default function RifaPage() {
  ...
  const handleDonationSubmit = useCallback(
    async (data: DonationFormData) => {
      ...
      try {
        await registerDonationAction(input, {
          actorId: user.id,
  ...
  const handleAssignTickets = useCallback(
    async (input: AssignTicketsInput) => {
      ...
      try {
        await assignTicketsAction(input, {
          actorId: user.id,
          actorName: user.name ?? undefined,
        });
        toast({
  ...
  const handleRunDraw = useCallback(() => {
    ...
    startTransition(async () => {
      try {
        await runDeterministicDrawAction(
          {
            campaignId: selectedCampaignId,
            seed: drawSeed,
            winnersCount,
  ...
  }, [drawSeed, drawWinnersCount, selectedCampaignId, startTransition, toast, user]);
